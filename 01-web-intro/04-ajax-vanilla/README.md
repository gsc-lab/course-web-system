...
페이지 새로고침 없이 JavaScript(XMLHttpRequest)로 서버 데이터를 비동기 요청하여 화면을 갱신한다.
...
브라우저 ─── index.html 로드 ──▶ Nginx
  │                               (:80)
  │
  └── JS: XMLHttpRequest ──▶ /api.php ──▶ PHP-FPM ──▶ MySQL
                                            (:9000)    (:3306)
          ◀── JSON 응답 ────────────────────┘
  │
  └── DOM 업데이트 (테이블 렌더링)
...
- **XMLHttpRequest**: 브라우저 내장 Ajax 객체 (fetch API의 전신)
...
└── src/
    ...
    └── index.html       # 프론트엔드 (XMLHttpRequest로 API 호출)
```
